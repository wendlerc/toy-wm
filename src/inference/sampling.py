...
def sample_with_grad(v, z, actions, num_steps=10, cfg=1.0, negative_actions=None, cache=None):
    ...
    for i in range(len(ts)-1):
        ...
        if negative_actions is None:
            uncond = v.action_emb.unconditional_action
            if actions.dim() == 3:
                negative_actions = uncond.to(actions).unsqueeze(0).unsqueeze(0).expand_as(actions)
            else:
                negative_actions = t.full_like(actions, fill_value=int(uncond), dtype=actions.dtype, device=device)
...
def sample_video(model, actions, n_steps=4, cfg=1.0, negative_actions=None, clamp=False, cache=None):
    ...
    frames = t.randn(batch_size, num_actions, model.in_channels, model.height, model.width, device=model.device, dtype=model.dtype)
    for aidx in range(num_actions):
        noise=t.randn(batch_size, 1, model.in_channels, model.height, model.width, device=model.device, dtype=model.dtype)
        z = sample(model, noise, actions[:, aidx:aidx+1], num_steps=n_steps, cfg=cfg, negative_actions=negative_actions, cache=cache)
        frames[:, aidx:aidx+1] = z.detach()
        if clamp:
