...
class CausalDit(nn.Module):
    def __init__(self, height, width, n_window, d_model, T=1000, in_channels=3,
        ...
        self.action_emb1 = nn.Embedding(n_actions, d_model)
        self.action_emb2 = nn.Embedding(n_actions, d_model)
    ...
    def forward(self, 
                ...
                actions: Union[Float[Tensor, "batch dur 2"], Float[Tensor, "batch dur d"]],
        ...
        if actions.shape[-1] == self.d_model:
            ...
            a = self.action_emb1(actions[:,:,0]) + self.action_emb2(actions[:,:,1]) # batch dur d
